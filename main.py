...
from filtros import filtrar_por_description, ordenar_por_preco, filtrar_por_id
...
def listar_produtos():
    ...
    lista_filtrada = products

    if preco_asc == "true":
        lista_filtrada = ordenar_por_preco(lista_filtrada, asc=True)
    elif preco_desc == "true":
        lista_filtrada = ordenar_por_preco(lista_filtrada, asc=False)
    elif description_part:
        lista_filtrada = filtrar_por_description(lista_filtrada, description_part)
...
def buscar_por_id(product_id):
    produto = filtrar_por_id(products, product_id)
    if produto:
